  ...
  const conclusion = `${sentence(
    choose(
      [
        ...
        `The middle card is doing most of the work: ${cardRef(cards[1].id)} decides the tone that ${cardRef(cards[2].id)} inherits`,
        `What ${cardRef(cards[0].id)} set in motion meets its test at ${cardRef(cards[1].id)}, and that test is the reading`,
        `Strip everything else away and the reading is about ${cardRef(cards[1].id)} — how it is handled shapes what the other two cards mean`,
      ],
      random,
    ),
  )} ${(() => {
    ...
    return sentence(choose(
      [
        ...
        `The simplest version of the spread says: ${pivotAction}, and let ${directionRef} respond to that instead of the other way around`,
        `What changes the tone of ${directionRef} is not more analysis but a willingness to ${pivotAction} and let the sequence do the rest`,
        `The practical thread is to ${pivotAction} — not as a final answer, but as the move that gives ${directionRef} something to build on`,
      ],
      random,
    ));
  })()}`.trim();
