    ...
    if (position === "center") {
      return pick(
        [
          ...
          `${lens} — this is the ${label} point ${cardName} brings into focus.`,
          `${lens}: at the center, ${cardName} makes the ${label} question concrete.`,
        ],
        random,
      );
    }
  ...
  if (position === "situation") {
    return pick(
      [
        ...
        `${cardName} establishes the first ${label} context: ${lens}.`,
        ...
        `${cardName} gives the first concrete shape to ${label}: ${lens}.`,
        `${cardName} begins the line by making ${label} about ${lens}.`,
        `The first thing to notice about ${label} is ${lens}, which ${cardName} brings forward.`,
        `${cardName} makes the opening conditions less abstract: ${label} is showing up as ${lens}.`,
      ],
      random,
    );
  }
  ...
  if (position === "pivot") {
    return pick(
      [
        ...
        `${cardName} shows where the ${label} question stops being theoretical: ${lens}.`,
        `The turn in the reading comes through ${lens}, and ${cardName} is what makes that visible.`,
        `At the hinge point, ${cardName} asks whether ${lens} can be handled plainly.`,
        `${cardName} makes the ${label} pressure practical rather than abstract: ${lens}.`,
      ],
      random,
    );
  }
  ...
  if (position === "direction") {
    return pick(
      [
        ...
        `${cardName} gives the next step its ${label} shape: ${lens}.`,
        `The way forward is not vague here; on ${label}, ${cardName} points to ${lens}.`,
        `As the line moves on, ${cardName} makes ${lens} the thing to follow.`,
        `The direction becomes clearer when ${label} is read as ${lens}.`,
      ],
      random,
    );
  }
  ...
  if (position === "center") {
    return pick(
      [
        ...
        `Most centrally, ${cardName} points to ${lens}.`,
        `This center placement gives the ${label} question a practical shape: ${lens}.`,
        `${cardName} makes the center of the reading specific: ${label} is really about ${lens}.`,
        `At the heart of the spread, ${cardName} turns ${label} toward ${lens}.`,
        `The center does not treat ${label} broadly; ${cardName} narrows it to ${lens}.`,
        `${cardName} is where the ${label} lens becomes concrete: ${lens}.`,
      ],
      random,
    );
  }
...
export function buildThemeSectionBridge(themeId: ThemeId | null, random: () => number): string {
  ...
  return pick(
    [
      ...
      `The cards do not mention ${label} only once; they keep returning to it from different angles.`,
      `The useful read is to let ${label} organize the details instead of treating each card as a separate clue.`,
      `${label} gives the spread its through-line, especially where the cards repeat the same pressure in different forms.`,
      `The more practical reading is to ask what ${label} changes in each section, not just whether it appears.`,
      `Across the layout, ${label} acts like a filter: it shows which details matter and which can be left aside.`,
      `The spread becomes clearer when ${label} is treated as an active question rather than a general topic.`,
    ],
    random,
  );
}
...
export function buildThemeConclusionLine(themeId: ThemeId | null, random: () => number): string {
  ...
  return pick(
    [
      ...
      `For ${label}, the answer is in the repeated pattern rather than a single dramatic card.`,
      `The useful response on ${label} is to follow the clearest repeated signal, not every possible implication.`,
      `The spread gives ${label} a workable shape: notice where it asks for attention, timing, or a cleaner next step.`,
      `On ${label}, the reading is strongest where it becomes concrete; that is the part to return to first.`,
    ],
    random,
  );
}
